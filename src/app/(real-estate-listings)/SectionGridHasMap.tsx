...
const SectionGridHasMap: FC<SectionGridHasMapProps> = () => {
  ...
  const [currentPage, setCurrentPage] = useState(1);
         const [filteredData, setFilteredData] = useState<StayDataType[]>(DEMO_EXPERIENCES);
          
          const totalPages = Math.ceil(DEMO_EXPERIENCES.length / ITEMS_PER_PAGE);
        ...
        <div className="min-h-screen w-full xl:w-[780px] 2xl:w-[880px] flex-shrink-0 xl:px-8 ">
          ...
          <div className="grid grid-cols-1 gap-8">
            {DEMO_EXPERIENCES.map((item) => (
              <div
                key={item.id}
                onMouseEnter={() => setCurrentHoverID(item.id as string | number)} // ✅ No unnecessary function wrapper
                onMouseLeave={() => setCurrentHoverID(-1)} // ✅ Directly set to -1
              >
