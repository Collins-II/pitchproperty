...
import { getPropertyBySearch } from "@/app/actions/getListings";
import { buildPropertySearchQuery } from "@/lib/utils";
import { PropertySidebar } from "./components/market-sidebar";
...
export default async function InventoryPage(props: PageProps) {
  try {
    ...
    const [classifieds, count] = await Promise.all([
      getPropertyBySearch(searchParams),
      Car.countDocuments(buildPropertySearchQuery(searchParams)),
    ]);
    ...
    return (
      <div className="flex">
        <PropertySidebar minMaxValues={minMaxResult} searchParams={searchParams} />
          ...
          <div className="flex space-y-2 items-center justify-between pb-4 -mt-1">
            <div className="flex justify-between items-center w-full">
              <h2 className="text-sm md:text-base lg:text-xl font-semibold min-w-fit">
                We have found {count} properties
              </h2>
              <DialogFilters
