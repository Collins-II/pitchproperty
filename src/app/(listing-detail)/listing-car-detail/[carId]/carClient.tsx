...
import RentalPickUpClient from "../RentalCarSearchClient";
...
import RentalAppointment from "../../(components)/RentalAppointment";
...
const ListingCarDetailPage: FC<ListingCarDetailPageProps> = ({carData, currentUser}) => {
  // USE STATE

  const thisPathname = usePathname();
  const router = useRouter();
  const [reservedDate, setReservedDate] = useState<Date>();
  ...
  const [bookingPeriod, setBookingPeriod] = useState<[Date | null, Date | null]>([startDate, endDate]);

  useEffect(() => {
    setBookingPeriod([startDate, endDate]);
  }, [startDate, endDate]);
  ...
  console.log("APP__DATA",appointmentData)
      ...
      <div className="listingSection__wrap !space-y-6">
        ...
        <div className="flex items-center">
          ...
          <span className="ml-2.5 text-neutral-500 dark:text-neutral-400">
            ...
            <span className="text-neutral-900 dark:text-neutral-200 font-medium">
             {currentUser?.name} {/* TESTING PURPOSES */}
            </span>
          </span>
        </div>
        ...
        <div className="flex items-center justify-between xl:justify-start space-x-8 xl:space-x-12 text-sm text-neutral-700 dark:text-neutral-300">
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 text-center sm:text-left sm:space-x-3 ">
            ...
            <span className=""> {carData.seats}</span>
          </div>
          <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 text-center sm:text-left sm:space-x-3 ">
  ...
  const renderSidebarDetail = () => {
    return (
      <div className="listingSection__wrap lg:shadow-xl w-full lg:w-96">
        ...
        <div className="ml-4 flex flex-col border border-neutral-200 dark:border-neutral-700 rounded-3xl ">
          <RentalPickUpClient pickUpLocation={pickUpLocation} setPickUpLocation={setPickUpLocation} />
          <div className="w-full border-b border-neutral-200 dark:border-neutral-700"></div>
          <RentalCarDatesRangeInput startDate={startDate} endDate={endDate} setStartDate={setStartDate} setEndDate={setEndDate} className="flex-1" />
        </div>
        </div>
      </div>
    );
    ...
    <div className={` nc-ListingCarDetailPage `}>
      ...
      <header className="rounded-md sm:rounded-xl">
        <div className="relative grid grid-cols-4 gap-1 sm:gap-2">
          ...
          {/*  */}
          {carData.galleryImgs.filter((_, i) => i >= 2 && i < 4).map((item, index) => (
            <div
              key={index}
