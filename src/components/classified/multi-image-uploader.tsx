...
export const MultiImageUploader = (props: MultiImageUploaderProps) => {
			...
			for (const file of files) {
				...
				uploader
					...
					.onComplete(() => {
						...
						const clone = items
							.concat(newImageData)
							.map((item: any) => ({ ...item, percentage: 100 }));

						setItems(clone);
						replace(clone.map((item: any) => ({ src: item.src, alt: item.alt })));
						setIsUploading(false);
					});
	...
	const remove = (i: number) => {
		setItems((prev: any) => prev.filter((item: any) => item.id !== i));
		replace(items.filter((item: any) => item.id !== i));
	};
